@extends('layouts.admin-master', ['pageName'=> 'section', 'title' => 'Edit section'])
            ...
            <div class="col-md-12">
                <div class="card heaight-auto">
                    <div class="d-flex justify-content-between heading card-header">
                        <h4 class=""><i class="fa fa-edit"></i> Edit section</h4>
                        <div>
                            ...
                            <form action="{{ route('section.update', $section) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-2">
                                        <div class="row my-2">
                                            ...
                                            <div class="col-md-9">
                                                <select class=" form-control form-control-sm" name="school_id" id="school_id" >
                                                    ...
                                                    <option value="{{$item->id}}" {{ $item->id == $section->school_id ? 'selected' : ''}}>{{$item->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row my-2">
                                            ...
                                            <div class="col-md-9">
                                                <select class=" form-control form-control-sm" name="class_id" id="class_id" >
                                                    ...
                                                    <option value="{{$item->id}}" {{ $item->id == $section->class_id ? 'selected' : ''}}>{{$item->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row my-2">
                                            ...
                                            <div class="col-md-9">
                                            <input class="form-control @error('title') is-invalid @enderror" id="title" type="text" name="title" value="{{ $section->title }}" placeholder="title">
                                            @error('title')
                                        ...
                                        <div class="row my-2">
                                            ...
                                            <div class="col-md-9">
                                                <textarea name="description" id="editor" cols="30" rows="10">{{ $section->description }}</textarea>
                                              @error('description')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                ...
                                <div class="clearfix mt-1">
                                    <div class="float-md-left">
                                        <a href="{{route('section.index')}}" id="prev" class="btn-fill-lg bg-blue-dark btn-hover-yellow">Back</a>
...
<script>
    ...
    document.getElementById("previewImage").src="{{ asset($section->image) }}";
</script>
<script>
    $(document).ready(function() {
            $("select[name='school_id']").on('change', function() {
             ...
             $.ajax({
                url: "{{url('/get-allclass')}}/" +school_id,
                dataType:'JSON',
                method:'GET',
