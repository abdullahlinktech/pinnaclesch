@extends('layouts.admin-master', ['pageName' => 'hostel', 'title' => 'Add Hostel'])
@push('admin-css')
@endpush
                        ...
                        <div class="card-body">
                            <div class="form">
                                <div class="d-flex justify-content-between heading card-header">
                                    <h4 class=""><i class="fas fa-plus"></i> Add Hostel</h4>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-2">
                                            <div class="row my-2">
                                                ...
                                                <div class="col-md-9">
                                                    <input id="radio1" type="radio" name="hotelType" value="b" checked>
                                                    <small for="">Boys</small>
                                                    <input type="radio" name="hotelType" value="g" >
                                            ...
                                            <div class="row my-2">
                                                <div class="col-md-3">
                                                    <label for="image" class=""> Image <small></small></label>
                                                </div>
                                                <div class="col-md-9">
                                                    <input
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                @forelse ($hostel as $key=>$item)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $item->title }}</td>
                                    <td>{{ $item->hotelType == 'b' ? 'Boys' : 'Girls'}}</td>
                                    <td><img class="border" style="height: 40px; width:50px;" src="{{ asset($item->image) }}" alt=""></td>
                                    <td>
                                        <a href="{{ route('hostel.edit',$item) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></a>
                                        <button type="submit" class="btn btn-danger btn-sm" onclick="deletePartner({{ $item->id }})"><i class="fa fa-trash"></i></button>
                                        <form id="delete-form-{{$item->id}}" action="{{route('hostel.delete',$item)}}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
